...
class DefaultExtension extends MProvider {
    ...
    async _processListingItems(doc) {
        ...
        for (const r of t) {
            ...
            if (t) {
                const i = t.getHref;
                const s = this._titleEdit(t.attr("title") || r.selectFirst("h3.title")?.text);
                const o = r.selectFirst("img")?.attr("data-src");
                if (i.includes("/series/")) {
                    try {
                        const t = await this.requestDoc(i.replace(this.source.baseUrl, ""));
                        const r = t.select("section.allseasonss div.Small--Box.Season");
                        if (r.length > 0) {
                            for (const t of r) {
                            ...
                            e.push({ name: s, imageUrl: o, link: i });
                        }
                    } catch (t) {
                        e.push({ name: s, imageUrl: o, link: i });
                    }
    ...
    async search(e, t, r) {
        ...
        const s = r[0];
        if (e) {
            i = `/search/?query=${encodeURIComponent(e)}&offset=${t-1}`;
        } else {
            const e = s.values[s.state].value;
            if (e) {
                i = `${e.endsWith("/")?e:e+"/"}page/${t}/`;
            } else {
                await this.getPopular(t)
            }
        }
    ...
    async getDetail(e) {
        ...
        const r = this._titleEdit(t.selectFirst("h1.post-title")?.text);
        const i = t.selectFirst("div.image img")?.getSrc;
        const s = t.selectFirst("div.story")?.text.trim();
        const o = t.select("div.catssection li a").map((e => e.text));
        const a = [];
        t.select("section.allepcont div.row a").forEach((e => {
            a.push({
    ...
    async _processLink(videoList, url, prefix) {
        ...
        if (url.includes("vidtube.pro/d/")) {
            embedUrl = url.replace("/d/", "/embed-");
        } else if (url.includes("updown.cam/")) {
            embedUrl = `https://updown.cam/embed-${url.split('/').pop()}.html`;
        } else if (url.includes("savefiles.com/")) {
            embedUrl = `https://savefiles.com/e/${url.split('/').pop()}`;
        }
        ...
        try {
            ...
            if (!foundVideos && hosterSelection.includes('other') && !prefix.startsWith('[DL]')) {
                ...
                videoList.push({ url: embedUrl, originalUrl: embedUrl, quality: quality });
            }
        } finally {
            ...
            if (videosAdded === 0 && this.getPreference("show_embed_url_in_quality")) {
                ...
                videoList.push({ url: "", originalUrl: embedUrl, quality: quality });
            }
        }
    }
    ...
    getFilterList() {
        const categories = [
            { "name": "اختر", "query": "" },
            { "name": "كل الافلام", "query": "/movies/" },
            { "name": "افلام اجنبى", "query": "/category/افلام-اجنبي-3/" },
            { "name": "افلام انمى", "query": "/category/افلام-انمي-1/" },
            { "name": "افلام اسيويه", "query": "/category/افلام-اسيوي/" },
            { "name": "افلام نتفليكس", "query": "/netflix-movies/" },
            { "name": "سلاسل الافلام", "query": "/movies/" },
            { "name": "الاعلي تقييما", "query": "/top-rating-imdb/" },
            { "name": "مسلسلات اجنبى", "query": "/category/مسلسلات-اجنبي/" },
            { "name": "مسلسلات اجنبى نتفليكس", "query": "/netflix-series/?cat=7" },
            { "name": "مسلسلات اسيوية", "query": "/category/مسلسلات-اسيوية-7/" },
            { "name": "مسلسلات اسيوية نتفليكس", "query": "/netflix-series/?cat=9" },
            { "name": "مسلسلات انمي", "query": "/category/مسلسلات-انمي-1/" },
            { "name": "مسلسلات انمي نتفلكس", "query": "/netflix-series/?cat=8" },
            { "name": "احدث حلقات الانمي", "query": "/category/مسلسلات-انمي-1/?key=episodes" }
        ];
        return [{
