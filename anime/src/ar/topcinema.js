...
class DefaultExtension extends MProvider {
    ...
    async _processLink(videoList, url, prefix) { const processedUrls = new Set(videoList.map(v => v.originalUrl)); if (!url || processedUrls.has(url)) return; const hosterSelection = this.getPreference("hoster_selection") || []; const initialVideoCount = videoList.length; let embedUrl = url; if (url.includes("vidtube.pro/d/")) embedUrl = url.replace("/d/", "/embed-"); else if (url.includes("updown.cam/")) embedUrl = `https://updown.cam/embed-${url.split('/').pop()}.html`; else if (url.includes("savefiles.com/")) embedUrl = `https://savefiles.com/e/${url.split('/').pop()}`; try { let foundVideos = false; const extractor = this.extractorMap.find(ext => hosterSelection.includes(ext.key) && ext.domains.some(d => embedUrl.includes(d))); if (extractor) { const extracted = await extractor.func.call(this, embedUrl, prefix); if (extracted.length > 0) { videoList.push(...extracted); foundVideos = true; } } if (!foundVideos && this.getPreference("use_fallback_extractor")) { const fallbackVideos = await this._allinoneExtractor(embedUrl, `[Fallback] ${prefix}`); if (fallbackVideos.length > 0) { videoList.push(...fallbackVideos); foundVideos = true; } } if (!foundVideos && hosterSelection.includes('other') && !prefix.startsWith('[DL]')) { let quality = `[Embed] ${prefix}`; if (this.getPreference("show_embed_url_in_quality")) quality += ` [${embedUrl}]`; videoList.push({ url: embedUrl, originalUrl: embedUrl, quality: quality }); } } finally { const videosAdded = videoList.length - initialVideoCount; if (videosAdded === 0 && this.getPreference("show_embed_url_in_quality")) { const quality = `[Debug Fail] ${prefix} [${embedUrl}]`; videoList.push({ url: "", originalUrl: embedUrl, quality: quality }); } } }
