...
class DefaultExtension extends MProvider {
    ...
    async _allinoneExtractor(url, prefix) {
        // Stage 1: Try all known specific extractors
        ...
        for (const extractorFunc of fallBackExtractors) {
            try {
                const videos = await extractorFunc.call(this, url, prefix);
                if (videos && videos.length > 0) { return videos; }
            } catch (e) { /* Continue to next extractor */ }
        }

        // Stage 2: If no specific extractor worked, try generic scraping
        try {
            const res = await this.client.get(url, this._getVideoHeaders(url));
            const body = res.body; const doc = new Document(body); const videoHeaders = this._getVideoHeaders(url);
