...
class DefaultExtension extends MProvider {
    ...
    _titleEdit(title) {
        ...
        e = e.replace(/[\u2013\u2014\u2015\u2212]/g, "-").replace(/\s*\(.*?\)\s*/g, " ").replace(/\s*\[.*?\]\s*/g, " "); let r = "";
        e = e.replace(/\b(\d{4})\b/, ((e, t) => (r = t, ""))).replace(/^(?:فيلم|عرض|مسلسل|برنامج|انمي)\s+/i, "");
        Object.keys(t).forEach((r => { const i = new RegExp(`الموسم\\s*(?:ال)*${r}\\b`, "gi"); e = e.replace(i, `الموسم ${t[r]}`) }));
        e = e.replace(/الموسم\s*(\d+)/gi, "s$1").replace(/الحلقة\s*(\d+)/gi, "E$1").replace(/\s+(?:مترجم|مترجمة|مدبلج|مدبلجة|اون لاين|اونلاين|كامل|بجودة|جودة|عالية|حصريا|مشاهدة)\s*$/gi, "").replace(/\b(?:HD|4K|FHD|UHD|HDRip|BluRay|WEB-DL|720p|1080p)\b/gi, "").replace(/\s+/g, " ");
    ...
    async _parseCataloguePage(doc, isSearch = false) { const list = []; const items = doc.select("div.Block--Item, div.Small--Box"); items.forEach(item => { const linkElement = item.selectFirst("a"); if (!linkElement) return; const name = this._titleEdit(linkElement.attr("title")); let imageUrlAttr = isSearch ? "src" : "data-src"; const imageUrl = item.selectFirst("img")?.attr(imageUrlAttr); const link = linkElement.getHref; list.push({ name, imageUrl, link }); }); const hasNextPage = !!doc.selectFirst("div.pagination ul.page-numbers li a.next"); return { list, hasNextPage }; }
    async getPopular(page) { const doc = await this.requestDoc(`/category/movies/?page=${page}`); return await this._parseCataloguePage(doc); }
    async getLatestUpdates(page) { const doc = await this.requestDoc(`/recent/page/${page}/`); return await this._parseCataloguePage(doc); }
    async search(query, page, filters) { let path; if (query) { path = `/?s=${encodeURIComponent(query)}&page=${page}`; } else { const categoryFilter = filters[0]; const selectedCategory = categoryFilter.values[categoryFilter.state].value; if (selectedCategory) { path = `/${selectedCategory}?page=${page}/`; } else { return this.getPopular(page); } } const doc = await this.requestDoc(path); return await this._parseCataloguePage(doc, !!query); }
    async getDetail(url) { const doc = await this.requestDoc(url.replace(this.source.baseUrl, '')); const name = this._titleEdit(doc.selectFirst("h1.post-title")?.text || "Unknown Title"); const imageUrl = doc.selectFirst("div.left div.image img")?.getSrc; const description = doc.selectFirst("div.story")?.text.trim(); const genre = doc.select("div.catssection li a").map(e => e.text); const chapters = []; const episodeElements = doc.select("section.allepcont div.row a"); if (episodeElements.length > 0) { const sortedEpisodes = [...episodeElements].sort((a, b) => { const numA = parseInt(a.selectFirst("div.epnum")?.text.replace(/\D/g, '').trim() || '0'); const numB = parseInt(b.selectFirst("div.epnum")?.text.replace(/\D/g, '').trim() || '0'); return numA - numB; }); sortedEpisodes.forEach(ep => { chapters.push({ name: this._titleEdit(ep.attr("title")), url: ep.getHref }); }); } else { chapters.push({ name: "مشاهدة", url: url }); } return { name, imageUrl, description, genre, status: 1, chapters, link: url }; }

    // --- VIDEO EXTRACTION CONTROLLER ---
    async getVideoList(url) {
        const allStreams = []; const fetchMode = this.getPreference("link_fetch_mode") || "both";
        ...
        const uniqueStreams = Array.from(new Map(allStreams.map(item => [item.url, item])).values()).filter(item => item.url);
    ...
    // --- LINK GATHERING ---
    async _getWatchLinks(url) { const videos = []; const watchUrl = url.endsWith('/') ? `${url}watch/` : `${url}/watch/`; try { const doc = await this.requestDoc(watchUrl.replace(this.getBaseUrl(), '')); for (const serverEl of doc.select("div.watch--servers--list ul li.server--item")) { await this._processLink(videos, serverEl.attr("data-link"), serverEl.text.trim()); } } catch (e) { console.error("Failed to get watch links:", e); } return videos; }
    async _getDownloadLinks(url) { const videos = []; const watchUrl = url.endsWith('/') ? `${url}watch/` : `${url}/watch/`; try { const doc = await this.requestDoc(watchUrl.replace(this.getBaseUrl(), '')); for (const downloadEl of doc.select("div.downloads a.download--item")) { await this._processLink(videos, downloadEl.getHref, `[DL] ${downloadEl.selectFirst("span")?.text.trim()}`); } } catch (e) { console.error("Failed to get download links:", e); } return videos; }
    ...
    async _upstreamExtractor(url, prefix) { try { const res = await this.client.get(url, this.getHeaders(url)); const script = res.body.substringAfter("eval(function(p,a,c,k,e,d)").substringBefore("</script>"); if (!script) return []; const unpacked = unpackJs("eval(function(p,a,c,k,e,d)" + script); const masterUrl = unpacked.match(/hls:\s*['"](https?:\/\/[^'"]+\.m3u8[^'"]*)['"]/)?.[1]; return masterUrl ? this._parseM3U8(masterUrl, prefix, this._getVideoHeaders(url)) : []; } catch (e) { return []; } }
    async _krakenfilesExtractor(url, prefix) { try { const res = await this.client.get(url, this.getHeaders(url)); const doc = new Document(res.body); const videoUrl = doc.selectFirst("source[src]")?.getSrc; return videoUrl ? [{ url: videoUrl, originalUrl: url, quality: this._formatQuality(prefix, videoUrl), headers: this._getVideoHeaders(url) }] : []; } catch (e) { return []; } }
    ...
    async _bigwarpExtractor(url, prefix) { try { const res = await this.client.get(url, this._getVideoHeaders(url)); const jwplayerSetup = res.body.substringAfter('jwplayer("vplayer").setup({').substringBefore('});'); if (!jwplayerSetup) return []; const videos = []; const sourceRegex = /{file:"([^"]+)",label:"([^"]+)"}/g; const matches = [...jwplayerSetup.matchAll(sourceRegex)]; for (const match of matches) { const videoUrl = match[1]; const qualityLabel = match[2]; const resolution = qualityLabel.split(' ')[0]; videos.push({ url: videoUrl, originalUrl: url, quality: this._formatQuality(`${prefix} - ${resolution}`, videoUrl), headers: this._getVideoHeaders(url) }); } return videos; } catch (e) { return []; } }

    // --- MASTER FALLBACK EXTRACTOR ---
    async _allinoneExtractor(url, prefix) {
        const fallBackExtractors = [ this._cybervynxExtractor, this._doodstreamExtractor, this._mixdropExtractor, this._vidguardExtractor, this._mp4uploadExtractor, this._voeExtractor, this._okruExtractor, this._vidmolyExtractor, this._uqloadExtractor, this._vkExtractor, this._videaExtractor, this._dailymotionExtractor, this._sendvidExtractor, this._streamtapeExtractor, this._streamwishExtractor, this._filemoonExtractor, this._vidbomExtractor, this._lulustreamExtractor, this._streamrubyExtractor, this._upstreamExtractor, this._krakenfilesExtractor, this._thetubeExtractor, this._bigwarpExtractor, ];
        for (const extractorFunc of fallBackExtractors) {
            try { const videos = await extractorFunc.call(this, url, prefix); if (videos && videos.length > 0) { return videos; } } catch (e) { /* Continue */ }
        }
        try {
            const res = await this.client.get(url, this._getVideoHeaders(url)); const body = res.body; const doc = new Document(body); const videoHeaders = this._getVideoHeaders(url);
            let sources = []; const directVideoSrc = doc.selectFirst("source[src]")?.getSrc || doc.selectFirst("video[src]")?.getSrc; if (directVideoSrc) { sources.push(directVideoSrc); }
            ...
            const urlRegex = /(https?:\/\/[^"' \s]+\.(?:m3u8|mp4|webm|mkv|mov|flv|avi))[^"' \s]*/ig; let match; while ((match = urlRegex.exec(potentialScripts)) !== null) { sources.push(match[0]); }
            const uniqueSources = [...new Set(sources.filter(s => s && s.startsWith("http")))]; const allVideos = [];
            for (const sourceUrl of uniqueSources) { if (sourceUrl.includes(".m3u8")) { allVideos.push(...await this._parseM3U8(sourceUrl, prefix, videoHeaders)); } else { allVideos.push({ url: sourceUrl, originalUrl: url, quality: this._formatQuality(prefix, sourceUrl, "Direct"), headers: videoHeaders }); } }
            return allVideos;
        } catch (e) { return []; }
    }
