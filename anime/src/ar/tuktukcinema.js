...
class DefaultExtension extends MProvider {
    ...
    _titleEdit(title) {
        ...
        let r = "";
        e = e.replace(/\b(\d{4})\b/, ((e, t) => (r = t, ""))); e = e.replace(/^(?:فيلم|عرض|مسلسل|برنامج|انمي)\s+/i, "");
        ...
        e = e.replace(/\b(?:HD|4K|FHD|UHD|HDRip|BluRay|WEB-DL|720p|1080p)\b/gi, ""); e = e.replace(/\s+/g, " ");
        if (r) { e += ` (${r})`; }
        return e.trim();
    }
    ...
    async search(query, page, filters) {
        ...
        else { const categoryFilter = filters[0]; const selectedCategory = categoryFilter.values[categoryFilter.state].value;
    ...
    async getDetail(url) {
        ...
        if (episodeElements.length > 0) {
            const sortedEpisodes = [...episodeElements].sort((a, b) => (parseInt(a.selectFirst("div.epnum")?.text.replace(/\D/g, '').trim() || '0') - parseInt(b.selectFirst("div.epnum")?.text.replace(/\D/g, '').trim() || '0')));
    ...
    async getVideoList(url) {
        ...
        uniqueStreams.sort((a, b) => { const aPreferred = a.quality.includes(preferredQuality); const bPreferred = b.quality.includes(preferredQuality); if (aPreferred && !bPreferred) return -1; if (!aPreferred && bPreferred) return 1; return 0; });
    ...
    async _getWatchLinks(url) {
        ...
        try { const doc = await this.requestDoc(watchUrl.replace(this.getBaseUrl(), '')); for (const serverEl of doc.select("div.watch--servers--list ul li.server--item")) { await this._processLink(videos, serverEl.attr("data-link"), serverEl.text.trim()); }
    ...
    async _getDownloadLinks(url) {
        const videos = [];
        const watchUrl = url.endsWith('/') ? `${url}watch/` : `${url}/watch/`;
        try { const doc = await this.requestDoc(watchUrl.replace(this.getBaseUrl(), '')); for (const downloadEl of doc.select("div.downloads a.download--item")) { await this._processLink(videos, downloadEl.getHref, `[DL] ${downloadEl.selectFirst("span")?.text.trim()}`); }
    ...
    async _processLink(videoList, url, prefix) {
        ...
        try {
            ...
            if (extractor) { const extracted = await extractor.func.call(this, url, prefix); if (extracted.length > 0) { videoList.push(...extracted); foundVideos = true; } }
            if (!foundVideos && this.getPreference("use_fallback_extractor")) { const fallbackVideos = await this._allinoneExtractor(url, `[Fallback] ${prefix}`); if (fallbackVideos.length > 0) { videoList.push(...fallbackVideos); foundVideos = true; } }
            if (!foundVideos && hosterSelection.includes('other') && !prefix.startsWith('[DL]')) { let quality = `[Embed] ${prefix}`; if (this.getPreference("show_embed_url_in_quality")) { quality += ` [${url}]`; } videoList.push({ url: url, originalUrl: url, quality: quality }); }
        } catch (e) { if (this.getPreference("show_embed_url_in_quality")) { videoList.push({ url: "", originalUrl: url, quality: `[Debug Fail] ${prefix} [${url}]` }); } }
    }

    // --- PRIMARY EXTRACTORS MAP ---
    extractorMap = [
        { key: 'cybervynx', domains: ['cybervynx.com', 'smoothpre.com'], func: this._cybervynxExtractor },
        { key: 'dood', domains: ["doodstream.com", "dood.to", "dood.so", "dood.cx", "dood.la", "dood.ws", "dood.sh", "doodstream.co", "dood.pm", "dood.wf", "dood.re", "dood.yt", "dooood.com", "dood.stream", "ds2play.com", "doods.pro", "ds2video.com", "d0o0d.com", "do0od.com", "d0000d.com", "d000d.com", "dood.li", "dood.work", "dooodster.com", "vidply.com", "d-s.io"], func: this._doodstreamExtractor },
    ...
    async _parseM3U8(playlistUrl, prefix, headers = {}) {
        ...
        try {
            ...
            let match; const parsedQualities = []; const baseUrl = playlistUrl.substring(0, playlistUrl.lastIndexOf('/') + 1);
            while ((match = regex.exec(masterPlaylistContent)) !== null) {
                ...
                let streamUrl = match[3].trim(); if (!streamUrl.startsWith('http')) { streamUrl = baseUrl + streamUrl; }
            ...
            if (parsedQualities.length > 0) { const finalVideos = [{ ...fallback[0] }]; finalVideos.push(...parsedQualities); return finalVideos; }
            return fallback;
        } catch (e) { console.error("M3U8 Quality Parse Error:", e); return fallback; }
    }

    // --- ALL-IN-ONE FALLBACK EXTRACTOR & MAP ---
    allinoneExtractorMap = [
        ...
        { key: 'streamwish', domains: ['streamwish.to', 'streamwish.fun', 'vidhide.fun', 'filelions.to'], func: this._streamwishExtractor },
        { key: 'filemoon', domains: ['filemoon.sx'], func: this._filemoonExtractor },
        { key: 'vidbom', domains: ['vidbom.com'], func: this._vidbomExtractor },
        { key: 'lulustream', domains: ['lulustream.com', 'luluvdo.com'], func: this._lulustreamExtractor },
        { key: 'mixdrop', domains: ["mixdrop.ps", "mixdrop.co", "mixdrop.to", "mixdrop.sx", "mixdrop.bz", "mixdrop.ch", "mixdrp.co", "mixdrp.to", "mixdrop.gl", "mixdrop.club", "mixdroop.bz", "mixdroop.co", "mixdrop.vc", "mixdrop.ag", "mdy48tn97.com", "md3b0j6hj.com", "mdbekjwqa.pw", "mdfx9dc8n.net", "mixdropjmk.pw", "mixdrop21.net", "mixdrop.is", "mixdrop.si", "mixdrop23.net", "mixdrop.nu", "mixdrop.ms", "mdzsmutpcvykb.net", "mxdrop.to"], func: this._mixdropExtractor },
        ...
        { key: 'bigwarp', domains: ['bigwarp.org'], func: this._bigwarpExtractor },
    ];

    async _allinoneExtractor(url, prefix) {
        try {
            // 1. Try to find a specific, high-quality extractor first
            const specificExtractor = this.allinoneExtractorMap.find(ext => ext.domains.some(d => url.includes(d)));
            if (specificExtractor) {
                return await specificExtractor.func.call(this, url, prefix);
            }
            
            // 2. If no specific extractor is found, fall back to the generic link finder
    ...
    async _vkExtractor(url, prefix) { const headers = this._getVideoHeaders(url); const res = await this.client.get(url, headers); const matches = [...res.body.matchAll(/"url(\d+)":"(.*?)"/g)]; const videos = matches.map(match => ({ url: match[2].replace(/\\/g, ''), originalUrl: url, quality: this._formatQuality(prefix, match[2], `${match[1]}p`), headers })); return videos.sort((a, b) => (parseInt(b.quality.match(/(\d+)p/)?.[1] || 0) - parseInt(a.quality.match(/(\d+)p/)?.[1] || 0))); }
    async _videaExtractor(url, prefix) { const res = await this.client.get(url, this.getHeaders(url)); const videoUrl = res.body.substringAfter("v.player.source(").substringBefore(");").match(/'(https?:\/\/[^']+)'/)?.[1]; return videoUrl ? [{ url: videoUrl, originalUrl: videoUrl, quality: this._formatQuality(prefix, videoUrl), headers: this._getVideoHeaders(url) }] : []; }
    ...
    async _sendvidExtractor(url, prefix) { const res = await this.client.get(url, this._getVideoHeaders(url)); const videoUrl = new Document(res.body).selectFirst("source#source-video")?.getSrc; return videoUrl ? [{ url: videoUrl, originalUrl: videoUrl, quality: this._formatQuality(prefix, videoUrl), headers: this._getVideoHeaders(url) }] : []; }
    ...
    async _vidbomExtractor(url, prefix) { const res = await this.client.get(url, this.getHeaders(url)); const script = res.body.substringAfter("sources: [").substringBefore("]"); if (!script) return []; const sources = script.split('{file:"').slice(1); let allVideos = []; for (const source of sources) { const src = source.substringBefore('"'); if (src.includes(".m3u8")) { allVideos.push(...await this._parseM3U8(src, prefix, this._getVideoHeaders(url))); } else { allVideos.push({ url: src, originalUrl: src, quality: this._formatQuality(`${prefix}: ` + source.substringAfter('label:"').substringBefore('"'), src), headers: this._getVideoHeaders(url) }); } } return allVideos; }
    ...
    async _streamrubyExtractor(url, prefix) { const res = await this.client.get(url, this.getHeaders(url)); const script = res.body.substringAfter("sources: [").substringBefore("]"); if (!script) return []; const urls = (script.match(/file:"([^"]+)"/g) || []).map(m => m.replace('file:"', '').replace('"', '')); let allVideos = []; for (const hlsUrl of urls) { if (hlsUrl.includes(".m3u8")) allVideos.push(...await this._parseM3U8(hlsUrl, prefix, this._getVideoHeaders(url))); } return allVideos; }
    async _upstreamExtractor(url, prefix) { const res = await this.client.get(url, this.getHeaders(url)); const script = res.body.substringAfter("eval(function(p,a,c,k,e,d)").substringBefore("</script>"); if (!script) return []; const unpacked = unpackJs("eval(function(p,a,c,k,e,d)" + script); const masterUrl = unpacked.match(/hls:\s*['"](https?:\/\/[^'"]+\.m3u8[^'"]*)['"]/)?.[1]; return masterUrl ? this._parseM3U8(masterUrl, prefix, this._getVideoHeaders(url)) : []; }
    async _krakenfilesExtractor(url, prefix) { const res = await this.client.get(url, this.getHeaders(url)); const videoUrl = new Document(res.body).selectFirst("source[src]")?.getSrc; return videoUrl ? [{ url: videoUrl, originalUrl: videoUrl, quality: this._formatQuality(prefix, videoUrl), headers: this._getVideoHeaders(url) }] : []; }
