...
class DefaultExtension extends MProvider {
    ...
    _getResolution(qualityLabel) {
        // Try to match common resolution patterns like 1080p, 720p, etc.
        const resolutionMatch = qualityLabel.match(/(\d{3,4})p/i); 
        if (resolutionMatch) {
            return parseInt(resolutionMatch[1], 10);
        }
        // If "Original" is in the label, give it a very high resolution value for sorting
    ...
    async getVideoList(url) {
        ...
        const topcinemaDownloadPageUrl = this.source.baseUrl + downloadPagePath;
        const downloadPageDoc = await this.request(topcinemaDownloadPageUrl, {
            "Referer": url // Referer from episode page to Topcinema download page
        ...
        if (!vidtubeLinkElement) {
            console.warn("No VidTube download link found on: " + topcinemaDownloadPageUrl);
        ...
        const vidtubeQualityPageDoc = await this.request(vidtubeQualityPageUrl, {
            // Referer from Topcinema download page to VidTube quality selection page
            "Referer": topcinemaDownloadPageUrl 
        }).then(res => new Document(res.body));
        ...
        relevantQualities.sort((a, b) => {
            const resA = this._getResolution(a.label); 
            ...
            return 0; // Maintain original order if resolutions are the same
        });
        ...
        for (const qualityEntry of relevantQualities) {
            try {
                ...
                const finalDownloadPageDoc = await this.request(qualityEntry.url, {
                    // Referer from VidTube quality selection page to specific VidTube quality page
                    "Referer": vidtubeQualityPageUrl 
                }).then(res => new Document(res.body));
                ...
                if (finalVideoUrl) {
                    streams.push({
                        ...
                        headers: {
                            "Referer": vidtubeOrigin, // Crucial for playing the video stream, as observed in original fetch
    ...
    getSourcePreferences() {
        return [{
            ...
            listPreference: {
                ...
                value: "Auto", // Default to Auto to return all available qualities
                entries: ["Auto", "Original", "1080p FHD", "720p HD", "480p SD"], 
                // Entry values are simplified for easier matching against quality labels.
                entryValues: ["Auto", "original", "1080p", "720", "480"], 
            }
        }, {
            ...
            multiSelectListPreference: {
                ...
                // Since only VidTube is currently supported, it's pre-selected.
                values: ["vidtube"], 
